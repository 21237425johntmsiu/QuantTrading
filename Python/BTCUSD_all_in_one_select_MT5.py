#!/usr/bin/env python3
"""
BTCUSD_all_in_one_select_MT5.py
Trade execution counterpart to BTCUSD_all_in_one_select.py.
Reads signals from btcusd_120960_signal table and executes trades via MetaTrader5.
Thresholds are driven by model_select from March_11_2026_comp66a820.npy.
"""
...
# ──────────────────── Configuration ────────────────────
from db_env import asset_path, db_config, env_get, mt5_credentials

DB_CONFIG = db_config(min_size=2, max_size=10)

SIGNAL_TABLE = "btcusd_120960_signal"
MODEL_SELECT_FILE = asset_path("March_11_2026_comp66a820.npy")
symbol = 'BTCUSD'
order_size = 0.01
...
def get_thresholds(model_select, model_len, side):
    ...
    if model_select is None:
        if side == 'buy':
            return (env_get("BUY_THRESHOLD", 0.70, float), env_get("BUY_THRESHOLD2", 0.70, float), 0.0)
        else:
            return (env_get("SELL_THRESHOLD", 0.21, float), env_get("SELL_THRESHOLD2", 0.41, float), 0.0)
...
async def main_loop():
    ...
    # Load model_select
    try:
        model_select = np.load(MODEL_SELECT_FILE)
        print(f"Loaded model_select from {MODEL_SELECT_FILE}: {len(model_select)} entries")
    except FileNotFoundError:
        model_select = None
        print("[model_select.npy not found — using default thresholds]")
        ...
        while True:
            ...
            if 'buy' in active_sides:
                ...
                if buy_signal is not None and stop_market():
                    ...
                    window = int(buy_signal['window'])
                    last_processed_buy_ts = int(buy_signal['open_timestamp'])

                    sig1_thresh, sig2_thresh, _sig3_thresh = get_thresholds(model_select, window, 'buy')
                    print(f"Buy signal check (window={window}): {singal1} >= {sig1_thresh}, {singal2} >= {sig2_thresh}")

                    if singal1 >= sig1_thresh and singal2 >= sig2_thresh:
            ...
            if 'sell' in active_sides:
                ...
                if sell_signal is not None and stop_market():
                    ...
                    window = int(sell_signal['window'])
                    last_processed_sell_ts = int(sell_signal['open_timestamp'])

                    sig1_thresh, sig2_thresh, _sig3_thresh = get_thresholds(model_select, window, 'sell')
                    print(f"Sell signal check (window={window}): {singal1} >= {sig1_thresh}, {singal2} >= {sig2_thresh}")
